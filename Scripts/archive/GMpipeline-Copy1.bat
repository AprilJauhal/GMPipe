...
## To run:
## $ bash path_to/GMPipe/Scripts/GMPipeline.bat path_to/GMPipeline_userinput.ctl
#-----------------------------------------------------
...
if test -f "${PIPE_PATH}/storage/opt_ML/concat/main_comparison.iqtree"; then
...
Rscript ${SCRIPT_PATH}/iqtree_writer2.R ${PIPE_PATH} ${IQTREE} ${MUSCLE}
	
timestamp=$(date +%F_%T)
echo "RUNNING IQTREE TESTING  " $timestamp
parallel -j${THREADS} < ${PIPE_PATH}/storage/ML/iq_list2.cmd

timestamp=$(date +%F_%T)
echo "COMPARING TESTS  " $timestamp
parallel -j${THREADS} < ${PIPE_PATH}/storage/ML/iq_list3.cmd
...
timestamp=$(date +%F_%T)
echo "SCRIPT FINISHED" $timestamp
