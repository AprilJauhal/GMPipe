...
ingroup_tbl_path <-paste0(PATH, "/storage/hmm/ingroup.tbl")
outgroup_tbl_path <-paste0(PATH, "/storage/hmm/outgroup.tbl")
histo_path <- paste0(PATH, "/out/bitscore_hist.png")
final_report_path <- paste0(PATH, "/out/FINAL_REPORT.txt")
...
write(as.vector(ingroup_score$bit_score), file=paste0(PATH, "/out/ingroup_bitscores.txt"), ncolumns=1)
write(as.vector(outgroup_score$bit_score), file=paste0(PATH, "/out/outgroup_bitscores.txt"), ncolumns=1)
...
graph1 <- gghistogram(histo_table,
                      x = c("tree_pass", "tree_fail", "tree_undet",
                            "ingroup", "outgroup"),
                      y = "..count..",
                      color = ".x.", fill = ".x.",     # color and fill by x variables
                      merge = TRUE,                    # Merge the 3 plots
                      xlab = "bit_score", 
                      palette = "npg"                  # Change color palette
)

ggarrange(graph1, ncol = 1) %>%
